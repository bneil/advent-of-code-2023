...
func pickFirstAndLast(str string) string {
	first := fmt.Sprintf(str[0:1])
	last := fmt.Sprintf(str[len(str)-1 : len(str)])
...
func main() {
	...
	for _, v := range final {
		num := clearString(v)
		if len(num) == 1 {
			num = fmt.Sprintf("%s%s", num, num)
		}
		if len(num) > 2 {
			num = pickFirstAndLast(num)
		}
